...
(function($) {
	$.fn.piroBox_content = function(opt) {
		opt = jQuery.extend({
		piro_speed : 700,
		bg_alpha : 0.9,
		piro_scroll : true,
	...
	$(opt.selector).each(function(rev){
		...
		current_class = current_class.replace('rs_plg_content','');
		current_class = current_class.replace('rsmg_content_first','');
		current_class = current_class.replace('rsmg_content_last','');
	...
	$(piro_prev).add(piro_next).bind('click',function(c) {
		...
		var obj_data = $(opt.selector).filter('.rsmg_content_item').attr('rev');
