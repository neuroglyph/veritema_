...
(function($) {
	$.fn.piroBox_system = function(opt) {
		opt = jQuery.extend({
		piro_speed : 700,
		bg_alpha : 0.9,
		piro_scroll : true,
	...
	for (var i=0; i<gall_settings.length; i++) {
		$(gall_settings[i]+':first').addClass('rsmg_system_first');
		$(gall_settings[i]+':last').addClass('rsmg_system_last');
	}
	
	var piro_gallery = $(my_gall_obj);
	var last_selector = '';
	var z = 0;
	$(opt.selector).each(function(rev){
		var current_class = $(this).attr('class');
		current_class = current_class.replace('rs_plg_system','');
		current_class = current_class.replace('rsmg_system_first','');
		current_class = current_class.replace('rsmg_system_last','');
		current_class = current_class.trim();
		
		if (current_class == last_selector){
			this.rev = current_class+'-'+z;
			z++;
		}
		else {
			z = 0;
	...
	$(piro_prev).add(piro_next).bind('click',function(c) {
		...
		var obj_data = $(opt.selector).filter('.rsmg_system_item').attr('rev'); /// aici verificat
	
		obj_data = obj_data.split('-');
		var obj_count = parseInt(obj_data[1]); //parseInt($(opt.selector).filter('.rsmg_content_item').attr('rev'));
		
		var find_obj = $(this).is(piro_prev) ? (obj_count - 1) : (obj_count + 1);
		$('a[class*="pirobox_gall"]').each(function() {
			if ($(this).attr('rev')==obj_data[0]+'-'+find_obj) {
				var start = $(this);
				start.click();
			}
		});		
	...
	$('html').bind('keyup' ,function(e) {
		 if ($('.rsmg_system_item').is('.rsmg_system_first')){
	...
	$('html').bind('keyup' ,function(z) {
		if ($('.rsmg_system_item').is('.rsmg_system_last')){
	...
	$(piro_gallery).each(function(){
		...
		var gal_type = $(this).attr('data-filter');
		...
		$(this).bind('click', function(e) {
			...
			$(piro_gallery).filter('.rsmg_system_item').removeClass('rsmg_system_item');
			$(this).addClass('rsmg_system_item');
			open_all();
			if($(this).is('.rsmg_system_first')){
				piro_prev.hide();
				piro_next.show();
			}else{
				piro_next.add(piro_prev).show();
			}
			if($(this).is('.rsmg_system_last')){
				if (!$(this).is('.rsmg_system_first'))
					piro_prev.show();
				piro_next.hide();	
			}
	...
	function open_all(){
				...
				case 'gallery':
					...
					if (typeof descr != 'undefined')
					{
						if(descr == "")
							...
							var piro_close_desc = $('<div id="piro_close_desc">x</div>');
							piro_capt.append(piro_close_desc);
						}
					}
					animate_image();
				break;
		...
		$('#piro_close_desc').live('click',function(){
