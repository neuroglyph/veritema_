...
class RSMediaGalleryModelAlbum extends JModelLegacy
{
	...
	{
		return urldecode(JRequest::getString('tag', 0));
	}
	...
	public function downloadItem()
	{
		if ($item = $this->getItem())
		{
			...
			$path = JPATH_SITE.'/components/com_rsmediagallery/assets/gallery/original/'.$item->filename;
	...
	public function getAlbumParams() {
		$albumId = JRequest::getCmd('Itemid', 0);
		...
		if (empty($params))
		{
			...
			$params 	= RSMediaGalleryHelper::parseParams($menu->getParams($albumId));
		}
